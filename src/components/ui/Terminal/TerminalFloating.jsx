...
const TerminalFloating = () => {
    ...
    return ( 
        <>
            ...
            <button 
                onClick={toggleOpen}
                className={`cursor-none terminal-toggle-btn fixed bottom-6 right-6 w-14 h-14 bg-black/40 hover:bg-black/60 backdrop-blur-xl border border-white/10 rounded-full flex items-center justify-center text-white shadow-2xl transition-all z-5 ${isOpen ? 'scale-0 opacity-0' : 'scale-100 opacity-100'}`}
                title="Open Terminal"
            >
                <MdTerminal size={28} />
            </button>
